...
Used to run mechanism analysis with RMS as an ideal gas in a batch reactor at constant V-U (adiabatic)
"""
...
class RMSSimulatorConstantUV(SimulateAdapter):
    """
    RMSSimulator is an adapter for the abstract class SimulateAdapter that simulates ideal gases
    ...
    def set_up(self):
        ...
        # obtain initial T, P since the initial V can be calculated implicitly by RMS
        ...
        self.domain, self.y0, self.p = rms.ConstantVDomain(phase=self.phase,
    ...
    def simulate(self):
        ...
        """
        self.logger.info('Simulating a job using RMS...')
        if not HAS_RMS:
        ...
        if len(self.observable_list):
            self.logger.info('Running SA using RMS...')
...
register_simulate_adapter("RMSSimulatorConstantUV", RMSSimulatorConstantUV)
