...
def test_set_up_no_sa():
    """
    Run RMG's minimal example without SA by testing the `set_up` method within the RMSConstantTP init method.
...
def test_get_sa_coefficients():
    """
    Run RMG's minimal example with SA by testing the `set_up` method within the RMSConstantTP init method.
    Then run the `get_sa_coefficients()` method to test that RMSConstantTP correctly parses the bsol object
    to obtain sa_dict.
    """
    t3 = run_minimal(project_directory=TEST_DIR)
    t3.set_paths()
