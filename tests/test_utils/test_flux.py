...
def test_create_digraph_NH3():
    ...
    observables = ['NH3(1)']
    profiles = flux.get_profiles_from_simulation(model_path=os.path.join(TEST_DATA_BASE_PATH, 'models', 'NH3.yaml'),
                                                 reactor_type='JSR',
                                                 times=[1e-2],
...
def test_create_digraph_N2H4():
    """Test creating a flux diagram of ammonia oxidation"""
    observables = ['H4N2(1)']
    profiles = flux.get_profiles_from_simulation(model_path=os.path.join(TEST_DATA_BASE_PATH, 'models', 'N2H4.yaml'),
                                                 reactor_type='JSR',
                                                 times=[0.05],
...
def test_get_node():
    ...
    node_f_str = node_f.to_string()
    assert '"Node F"' in node_f_str
    assert 'style=filled' in node_f_str
    assert 'fillcolor="#DCE5F4"' in node_f_str
...
def test_get_flux_graph(hocho_simulation_data):
    ...
    for i, (time, profile) in enumerate(profiles.items()):
        ...
        assert len(profile['ROPs']) == 36  # number of species in the N2H4 model
...
def teardown_function(function):
    """Remove any scratch artifacts created by the just-finished test."""
