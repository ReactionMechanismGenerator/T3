...
def append_to_rmg_library(
    ...
    try:
        with open(from_py_path, "r", encoding="utf-8") as f:
            source_lib = shim.parse_rmg_library(f.read())
        with open(to_py_path, "r", encoding="utf-8") as f:
            destination_lib = shim.parse_rmg_library(f.read())
    ...
    entries_to_add: list[shim.Entry] = []
    ...
    if lib_type == "thermo":
        ...
        if entries_to_add:
            ...
            content = shim.write_library_to_string(destination_lib)
