...
class RMGConstantTP(SimulateAdapter):
    ...
    def get_sa_coefficients(self) -> Optional[dict]:
        ...
        sa_dict = {
            'kinetics': dict(),
            'thermo': dict(),
            'time': np.array(raw_sa_dict.get('time', []))
        }

        if 'kinetics' in raw_sa_dict:
            for obs_label, param_dict in raw_sa_dict['kinetics'].items():
                if obs_label not in sa_dict['kinetics']:
                    sa_dict['kinetics'][obs_label] = dict()
                ...
                for rmg_index, values in param_dict.items():
                    ...
                    try:
                        ...
                        if chem_index is not None:
                            sa_dict['kinetics'][obs_label][chem_index] = arr_values
                        else:
                            sa_dict['kinetics'][obs_label][rmg_index] = arr_values
                    except ValueError:
                        sa_dict['kinetics'][obs_label][rmg_index] = arr_values

        if 'thermo' in raw_sa_dict:
            for obs_label, param_dict in raw_sa_dict['thermo'].items():
                if obs_label not in sa_dict['thermo']:
                    sa_dict['thermo'][obs_label] = dict()
                for spc_label, values in param_dict.items():
                    sa_dict['thermo'][obs_label][spc_label] = np.array(values)
