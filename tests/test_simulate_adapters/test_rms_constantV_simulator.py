...
"""
t3 tests test_rms_constantV_simulator module
"""
...
from t3.simulate.rms_constantV_simulator import RMSConstantV


TEST_DIR = os.path.join(SIMULATE_DATA_BASE_PATH, 'rms_constantV_simulator_test')


def test_set_up_no_sa():
    """
    Run RMG's minimal example without SA by testing the `set_up` method within the RMSConstantV init method.
    ...
    RMSSimulator_adapter = RMSConstantV(t3=t3.t3,
                                        rmg=t3.rmg,
                                        paths=t3.paths,
                                        logger=t3.logger,
...
def test_get_sa_coefficients():
    """
    Run RMG's minimal example with SA by testing the `set_up` method within the RMSConstantV init method.
    Then run the `get_sa_coefficients()` method to test that RMSConstantV correctly parses the bsol object
    to obtain sa_dict.
    """
    t3 = run_minimal(project_directory=TEST_DIR)
    t3.set_paths()
