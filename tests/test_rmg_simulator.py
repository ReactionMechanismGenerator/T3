...
def test_set_up_no_sa():
    ...
    rmg_simulator_adapter = RMGSimulator(t3=t3.t3,
                                         rmg=t3.rmg,
                                         paths=t3.paths,
                                         logger=t3.logger,
...
def test_get_sa_coefficients():
    """
    Run RMG's minimal example with SA by testing the `set_up` method within the RMGSimulator init method.
    Then run the `get_sa_coefficients()` method to test that RMGSimulator correctly parses the SA csv files
    to obtain sa_dict.
    """
    t3 = run_minimal(project_directory=TEST_DIR)
    t3.set_paths()
