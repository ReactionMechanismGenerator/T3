...
def _hindered_rotor_dict(mode) -> dict:
    """Serialize a HinderedRotor into the kwargs an inline RMG mode needs.

    HinderedRotor only, deliberately: the hybrid writer's own validation accepts
    ``type == 'HinderedRotor'`` and nothing else, so advertising FreeRotor here would promise an
    input that is refused one step downstream."""
