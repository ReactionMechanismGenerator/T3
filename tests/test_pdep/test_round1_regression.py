...
from t3.pdep.parser import parse_pdep_network_file, strip_rmg_index_suffix  # noqa: F401  (strip_* re-exported for mutation reference)
...
def round1_handoff(tmp_path_factory):
    ...
    conformers = json.load(open(CONFORMERS_JSON))
...
def test_rate_coefficient_matches(round1_handoff):
    ...
    * Therefore: a mismatch WITHIN 1% means "the numbers legitimately moved" -- most likely an
    ...
    """
    text = open(round1_handoff['output_py_path']).read()
...
def test_generated_hybrid_is_dof_consistent(round1_handoff):
    ...
    text = open(round1_handoff['hybrid_path']).read()
