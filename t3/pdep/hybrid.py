...
def write_hybrid_network_input_file(source_path: str,
    ...
    # Isomers, NOT species_labels. `species_labels` is every `species(...)` block in the file,
    # which includes the bath gas and the bimolecular reactant/product channel species -- on the
    # CHO2 network that is `Ar`, `O=C=O(5)` and `[H](6)` alongside the two real wells. Validating
    # against it would accept `qm_wells={'Ar': ...}` and rewrite the BATH GAS's E0 and modes.
    # Nothing downstream would catch that: the write-time DOF invariant deliberately skips
    # non-isomer species, because channel species legitimately carry full conformers. Only an
    # isomer is a well, so only an isomer can be adopted as one.
    known_isomer_labels = set(network.isomers)
    unknown_well_labels = sorted(set(qm_wells) - known_isomer_labels)
    if unknown_well_labels:
        raise ValueError(f"qm_wells references label(s) {unknown_well_labels} that are not isomers (wells) of "
                         f"network '{network.network_id}'; its isomer labels are {sorted(known_isomer_labels)}. "
                         f"Only an isomer can be adopted as a well -- bath-gas and bimolecular channel species "
                         f"are declared with species(...) too, but are not wells.")
